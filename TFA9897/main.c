...
Tfa9890_Error_t Tfa_generic_start(uint16_t slave_address, uint8_t calibrate)
{
	...
	assert(error == Error_Ok);
	error = tfa_set_bf(slave_address, TFA1_BF_TDMSAMSZ, 15);//32 FS(15)
	assert(error == Error_Ok);
	error = tfa_set_bf(slave_address, TFA1_BF_NBCK, 0);//  
	assert(error == Error_Ok);
	...
	error = tfa_set_bf(slave_address, TFA1_BF_IPLL, 0);//
	assert(error == Error_Ok);
	...
	TFA98XX_PRINT("TFA9897 0x%02X Calibration value is %2.2f ohm\n", slave_address, re25);
