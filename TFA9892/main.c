...
{
	{0x000f, 0x0255}, //POR=0x0245
	{0x010f, 0x838a}, //POR=0x83ca
	{0x020f, 0x2dc8}, //POR=0x2828
	{0x050f, 0x762a}, //POR=0x766a
	{0x220f, 0x543c}, //POR=0x545c
	{0x260f, 0x0100}, //POR=0x0010
	{0x510f, 0x0000}, //POR=0x0080
	{0x520f, 0x551c}, //POR=0x1afc
	{0x530f, 0x003e}, //POR=0x0018
	{0x610f, 0x000c},
	{0x630f, 0x0a96}, //POR=0x0a9a
	{0x650f, 0x0a82}, //POR=0x0a8b
	{0x660f, 0x0701},
	{0x6c0f, 0x01d5}, //POR=0x02d5
	{0x700f, 0x26f8}, //POR=0x06e0
	{0x710f, 0x3074}, //POR=0x2074
	{0x750f, 0x4484}, //POR=0x4585
	{0x760f, 0x72ea}, //POR=0x54a2
	{0x830f, 0x0716},
	{0x890f, 0x0013}, //POR=0x0014
	{0xb00f, 0x4c08},
	{0xc60f, 0x004e},
	{TFA2_BF_TDMNBCK,0},
	{TFA2_BF_TDMSLLN,15},
...
int tfa_generic_bypass(uint16_t slave_address)
{
	uint16_t regRead = 0;
...
{
	uint8_t trial = 10, ManagerState = 10, sc = 0, cnt = 0, tries = 0;
	...
	while (tfa_get_bf_tfa98xx(slave_address, TFA2_BF_MANSTATE) < 6)
	{
		TFA98XX_PRINT("Waiting for INITCF mode, MANSTATE:%d\n", tfa_get_bf_tfa98xx(slave_address, TFA2_BF_MANSTATE));
		TFA98XX_SLEEP(10);
		if (ManagerState-- == 0) {
			TFA98XX_PRINT("Cannot reach INITCF state\n");
			return Error_StateTimedOut;
		}
	}
	TFA98XX_PRINT("MANAGER STATE OK(ManagerStateTrial = %d)\n", ManagerState);
	...
	assert(error == Error_Ok);
	error = tfa_write_tables_tfa98xx(slave_address);
	assert(error == Error_Ok);
	...
	error = tfa_dsp_msg_tfa98xxx(slave_address, SetVddpConfig_msg_len, SetVddpConfig_msg);
	assert(error == Error_Ok);
	TFA98XX_PRINT("MessageOk13\n");
	...
	TFA98XX_PRINT("TFA9892 0x%02X Calibration value is %2.2f mohm\n", slave_address, re25);
	error = tfa_set_bf_tfa98xx(slave_address, TFA2_BF_SSFAIME, 0);
	TFA98XX_PRINT("MTPOTC :%d -- MTPEX:%d:--SSFAIME:%d\n", tfa_get_bf_tfa98xx(slave_address, TFA2_BF_MTPOTC), tfa_get_bf_tfa98xx(slave_address, TFA2_BF_MTPEX), tfa_get_bf_tfa98xx(slave_address, TFA2_BF_SSFAIME));
	return error;
